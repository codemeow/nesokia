...
void nsk_pattable_show(
    ...
    for (size_t ty = 0; ty < NSK_PATTABLETABLE_HEIGHT; ty++) {
        nsk_inf(
            "  $%03zx- ",
            nsk_conv_pataddress2value(table->address) / 0x10 + ty
        );
...
void nsk_pattable_swapaddress(
    ...
    enum nsk_pattable_address buf =
        tables->plane[NSK_PLANE_BACKGROUND].address;

    tables->plane[NSK_PLANE_BACKGROUND].address =
        tables->plane[NSK_PLANE_SPRITES].address;
